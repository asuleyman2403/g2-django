...
def add_to_basket_view(request, pk):
    if request.user and request.user.is_authenticated:
        try:
            product = Product.objects.get(id=pk)
            try:
                prev_basket_item = BasketItem.objects.get(owner_id=request.user.id, product_id=product.id)
                prev_basket_item.amount += 1
                prev_basket_item.save()
            except BasketItem.DoesNotExist as e:
                basket_item = BasketItem(owner_id=request.user.id, product_id=product.id, amount=1)
                basket_item.save()
            return redirect(to='basket_page')
        except Product.DoesNotExist as e:
